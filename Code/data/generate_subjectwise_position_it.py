"""
This code is to be run if you are running the iterative brain function representation method of data preparation.
"""
import os
...
base_dir = '../../Data_Preparation/Result_FCandSignal_BasedPatch_Anchor/NYU_PatchSize_Variations'
iterations = [f'Iteration_{i}' for i in range(1, 4)]
data_fold = pd.read_csv(f'./NYU171.csv', skip_blank_lines=True)

os.makedirs('subjectwise_position_iterative', exist_ok=True)
os.makedirs('FCmatrix_iterative', exist_ok=True)
...
for index, row in data_fold.iterrows():
    ...
    for iteration in iterations:
        FCMatrix_path = glob(
            os.path.join(base_dir, iteration, 'FCMatrix', f'NYU_00{name}_func_preproc.mat')
        )
        Posi_Signal_path = glob(
            os.path.join(base_dir, iteration, 'Position_and_ROISignals', f'NYU_00{name}_func_preproc.mat')
        )
    ...
    if all_FCMatrices:
        ...
        np.save(os.path.join('subjectwise_position_iterative', f'{name}.npy'), posi)

        subject_fc_dir = os.path.join('./FCmatrix_iterative', str(name))
