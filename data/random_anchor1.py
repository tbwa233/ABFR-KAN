...
anchor_path = './AnchorMask/ModifiedCoordinateWithGMmask_forAnchorSize32'
os.makedirs(f"{anchor_path}", exist_ok=True)
np.save(f'{anchor_path}/Modified_AnchorPatch_index.npy', label_index_array)
np.save(f'{anchor_path}/Modified_AnchorPatch_mask.npy', wholefinal)

df = pd.DataFrame(label_index_array)
df.to_csv(f'{anchor_path}/Modified_AnchorPatch_index.csv')
...
sitk.WriteImage(whole_image, f'{anchor_path}/Modified_AnchorPatch_mask_181217181.nii')

volume = sitk.ReadImage(f'{anchor_path}/Modified_AnchorPatch_mask_181217181.nii')
...
sitk.WriteImage(resliced_mask, f'{anchor_path}/Modified_AnchorPatch_mask_{label-1}AnchorNum_617361.nii')
