...
# Updated output directory and file paths
result_dir = './Result_FCandSignal_BasedPatch_Anchor/Modified_NYU_PatchSize8_112AnchorNum'
image_dir = './NYU'
mask_dir = './template'
name_list = os.listdir(f'{image_dir}')
name_list.sort()

# Updated anchor mask file path
anchor_arr_3 = sitk.GetArrayFromImage(sitk.ReadImage(
    './AnchorMask/ModifiedCoordinateWithGMmask_forAnchorSize32/Modified_AnchorPatch_mask_150AnchorNum_617361.nii'))
...
for name in name_list:
    ...
    io.savemat(os.path.join(result_path, f'Modified_{name[:-7]}.mat'), {'cc_matrix': cc_matrix})
    signal_path = os.path.join(f'{result_dir}/Position_and_ROISignals')
    os.makedirs(signal_path, exist_ok=True)
    io.savemat(os.path.join(signal_path, f'Modified_{name[:-7]}.mat'), {'Position_and_ROISignals': result})
