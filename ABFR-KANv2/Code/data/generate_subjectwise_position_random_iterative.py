...
os.makedirs('FCmatrix_random_iterative', exist_ok=True)
...
for index, row in data_fold.iterrows():
    ...
    if all_FCMatrices:
        ...
        np.save(os.path.join('subjectwise_position_random_iterative', f'{name}.npy'), posi)

        subject_fc_dir = os.path.join('./FCmatrix_random_iterative', str(name))
