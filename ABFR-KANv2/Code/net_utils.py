...
def val_epoch(model, loader, device, lossfunction, return_logits=False):
    model.eval()
    total_loss, total_num = 0.0, 0
    pred_list, target_list = [], []
    preds_score_list = []
    logits_list = []  # Add this to store raw logits
    ...
    with torch.no_grad():
        for batch_idx, (fc_matrixs, targets, posi) in enumerate(loader):
            ...
            if return_logits:
                logits_list.append(out_1.cpu().numpy())  # Store raw logits
